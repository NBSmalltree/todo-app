...
- **颜色标签**：点击任务左侧颜色圆点，循环切换红/橙/黄/绿/无，快速标记紧急程度
- **截止日期**：为任务设置截止日期（精确到分钟），过期红色提醒、当天黄色提醒
- **未来安排**：设定未来日期，到期前任务自动隐藏，到期后自动显示
- **番茄钟专注**：内置番茄钟计时器（可配置时长），关联任务，专注/休息循环，运行时标题栏显示倒计时
- **注释备注**：双击任务或归档记录可添加备注
- **动画效果**：任务添加滑入、勾选脉冲、删除滑出、拖拽缩放阴影等平滑动画
- **右键菜单**：标记完成、归档（仅限已完成任务）、删除
- **已完成折叠**：已完成任务默认折叠为「已完成 N 项」，点击展开，保持列表清爽
- **删除撤销**：删除或归档操作后底部出现撤销按钮，5 秒内可恢复
- **快速筛选**：标题栏下方「全部/今天/过期/无截止日期」一键筛选，直观显示当前数量
- **搜索增强**：Ctrl/Cmd+F 切换搜索面板，支持关键词搜索 + 分类筛选 + 日期状态筛选
...
- **全局快捷键**：Cmd/Ctrl + Shift + T 显示/隐藏窗口、Cmd/Ctrl + Shift + Space 快速添加任务、Cmd/Ctrl + Shift + A 打开历史归档（均支持自定义）
- **窗口快捷键**：Cmd/Ctrl + F 切换搜索、Cmd/Ctrl + , 打开设置
...
### 当前版本
**v1.4.0** - 全新清爽布局、已完成任务折叠、删除撤销、快速筛选栏、颜色标签快捷切换、搜索增强。
...
2. 点击左侧圆圈标记完成（带脉冲动画）；点击颜色圆点循环切换红/橙/黄/绿/无紧急度
...
7. **番茄钟专注**：点击任务右侧的番茄钟图标，启动专注计时（可配置时长），运行时标题栏显示倒计时
8. **搜索筛选**：按 Ctrl/Cmd+F 切换搜索面板，支持关键词、分类和日期状态筛选（今天/过期/无截止日期）
9. **快速筛选**：标题栏下方的「全部/今天/过期/无截止日期」按钮一键切换视图
10. **批量操作**：点击标题栏「选择」按钮，进入批量选择模式，支持批量删除、批量归档
11. 右键已完成任务可选择「归档」（未完成任务不允许归档）；操作后底部出现撤销按钮可恢复
12. 已完成任务默认折叠，点击「已完成 N 项」展开
13. Ctrl/Cmd + 滚轮缩放窗口，拖动四角调整比例
14. 点击其他区域或按 ESC 可退出搜索、日期选择器和批量选择模式
...
│   │   ├── database.js    # SQLite 数据库（任务 CRUD、设置、备份恢复、迁移）
